...
["Category", "Time (ms)"],
  (
    ...
    # remove the "idle" object as no longer required for this chart, calculate the values and add on the 'other' category at the end
    | ($obj | del(.Idle) | to_entries[] | [.key, (.value | .. | add?)]),["Other", $other]
  )
  # format as a CSV
  | @csv
